...
def parse_args():
    ...
    # HED  (gradient-mask → NMS → threshold → directed bridge pipeline)
    p.add_argument('--hed-threshold',      type=float, default=0.25,
                   help='Hard threshold on the NMS-thinned, gradient-mask-refined HED map. '
                        'Pixels below this value are discarded before bridging.  '
                        'The gradient-mask filter already suppresses low-gradient noise, '
                        'so this can be set lower than a raw HED threshold would allow.  '
                        'Lower = longer surviving fragments, more bridging opportunities.  '
                        'Default: 0.40.')
    p.add_argument('--hed-threshold-high', type=float, default=0.90,
                   help='(Unused in the current HED path.)  Retained for compatibility.')
    p.add_argument('--hed-blur',           type=int,   default=1,
                   help='(Unused.)  Retained for argparse compatibility; the '
                        'HED pipeline no longer blurs.')
    ...
    # HED directed endpoint bridging
    p.add_argument('--hed-bridge-kernel',   type=int,   default=12,
                   help='Max endpoint gap to bridge in pixels (1=disabled).  '
                        'Bridges only form where the soft HED map supports the path.')
    p.add_argument('--hed-bridge-angle',    type=float, default=30.0,
                   help='Half-angle of tangent search cone in degrees.  '
                        'Lower = stricter collinearity; higher = accepts wider curves.')
    p.add_argument('--hed-bridge-lookback', type=int,   default=3,
                   help='Pixels to trace back along the chain for tangent estimation.')
    p.add_argument('--hed-bridge-soft-th',  type=float, default=0.15,
                   help='Min gradient-refined HED probability required along a bridge '
                        'path (checked against the `refined` map, not raw HED).')
    p.add_argument('--hed-bridge-co-circ',  type=float, default=10.0,
                   help='Max |α−β| in degrees for co-circularity arc acceptance.  '
                        'Allows bridging arcs where both endpoints deviate equally '
                        'from the chord (circles), even beyond --hed-bridge-angle.')
    # EDTER
...
def _endpoint_tangent(edge_bool, x, y, lookback, H, W):
    """Return the outward unit tangent at skeleton endpoint (x, y) by tracing back
    `lookback` pixels along the connected chain.  Returns None for isolated points."""
    path = [(x, y)]
    visited = {(x, y)}
        ...
        for dy in (-1, 0, 1):
            for dx in (-1, 0, 1):
                if dx == 0 and dy == 0:
                    continue
                nx, ny = cx + dx, cy + dy
                if (nx, ny) in visited or not (0 <= ny < H and 0 <= nx < W):
                    continue
                if edge_bool[ny, nx]:
                    path.append((nx, ny))
                    visited.add((nx, ny))
    ...
    if len(path) < 2:
        return None
    tx, ty = path[0][0] - path[-1][0], path[0][1] - path[-1][1]
    d = math.sqrt(tx * tx + ty * ty)
    return (tx / d, ty / d) if d > 1e-6 else None


def _directed_endpoint_bridge(binary, soft_f32, bridge_kernel, bridge_angle_deg,
                               lookback, bridge_soft_th, bridge_co_circ_deg):
    """
    Close 1–N pixel gaps between NMS skeleton endpoints using a directed
    compatibility graph and recursive chain assembly.

    Phase 1: Build a directed graph where nodes are degree-1 skeleton pixels
    (endpoints) and edges are valid bridge candidates.  A candidate (A, B) is
    accepted when both endpoints' tangents point toward each other within the
    configured angle tolerance (straight or co-circular arc test) AND the
    gradient-refined probability map has continuous support along the bridge.

    Phase 2: Score each bridge by looking ahead depth=3 hops along the chain it
    would initiate, so a slightly wider gap that leads to a long coherent chain
    is preferred over the nearest dead-end match.  Bridges are then committed
    greedily from highest to lowest total score, with each endpoint allowed in
    at most one bridge — preventing Y-forks in the resulting skeleton.

    binary          : uint8 [H, W] 1-pixel skeleton (NMS + threshold)
    soft_f32        : float32 [H, W] gradient-refined HED probability map
    bridge_kernel   : maximum gap in pixels (search radius); 1 = disabled
    bridge_angle_deg: half-angle for straight-edge acceptance (degrees)
    lookback        : chain pixels to trace for tangent estimation
    bridge_soft_th  : min soft_f32 value required along bridge path
    bridge_co_circ_deg: max |α−β| for co-circularity arc acceptance (degrees)
    """
    H, W = binary.shape
    if bridge_kernel <= 1:
        return binary
    ...
    # Endpoint pixels: exactly 1 eight-connected neighbor in the skeleton
    k8 = np.ones((3, 3), dtype=np.float32)
    k8[1, 1] = 0.0
    nbr = cv2.filter2D(edge_bool.astype(np.float32), -1, k8,
                       borderType=cv2.BORDER_CONSTANT)
    ep_ys, ep_xs = np.where(edge_bool & (nbr.astype(np.uint8) == 1))
    N = len(ep_ys)
    if N < 2:
        return binary

    cos_thr      = math.cos(math.radians(bridge_angle_deg))
    cos_2thr     = math.cos(math.radians(min(89.9, 2.0 * bridge_angle_deg)))
    co_circ_rad  = math.radians(bridge_co_circ_deg)
    bk2          = float(bridge_kernel * bridge_kernel)

    ep_xy  = [(int(ep_xs[i]), int(ep_ys[i])) for i in range(N)]
    ep_arr = np.array(ep_xy, dtype=np.float64)

    tangents = [
        _endpoint_tangent(edge_bool, x, y, lookback, H, W)
        for x, y in ep_xy
    ]

    # ── Phase 1: build compatibility graph ───────────────────────────────────
    # adj[i] = list of (j, quality) for each accepted bridge from endpoint i
    ...
    for i in range(N):
        ...
        near   = np.where((dists2 > 0) & (dists2 <= bk2))[0]
        ...
        for j in near:
            ...
            # α: angle between T_A and chord A→B
            dot_a  = max(-1.0, min(1.0, ta[0] * ux + ta[1] * uy))
            alpha  = math.acos(dot_a)
            # β: angle between T_B and chord B→A
            dot_b  = max(-1.0, min(1.0, tb[0] * (-ux) + tb[1] * (-uy)))
            beta   = math.acos(dot_b)

            # Acceptance: straight/mildly-curved OR co-circular arc
            straight_ok = dot_a >= cos_thr and dot_b >= cos_thr
            arc_ok      = (abs(alpha - beta) <= co_circ_rad
                           and dot_a >= cos_2thr and dot_b >= cos_2thr)
            if not (straight_ok or arc_ok):
                continue

            # Soft-map support along the straight bridge path
            n_s = max(3, int(dist) + 1)
            ok  = True
            for k_s in range(n_s):
                t  = k_s / (n_s - 1)
                sx = int(round(ax + t * dx))
                sy = int(round(ay + t * dy))
                if 0 <= sy < H and 0 <= sx < W:
                    if soft_f32[sy, sx] < bridge_soft_th:
                        ok = False
                        break
            if not ok:
                continue

            # Quality: higher alignment and shorter distance → higher score
            quality = (dot_a + dot_b) * 0.5 - dist / bridge_kernel
            adj[i].append((j, quality))

    # ── Phase 2: recursive chain quality (lookahead depth = 3) ───────────────
    DEPTH = 3

    def _lookahead(ep_idx, incoming_t, depth, visited):
        if depth == 0:
            return 0.0
        best = 0.0
        for (j, q) in adj[ep_idx]:
            if j in visited:
                continue
            ax2, ay2 = ep_xy[ep_idx]
            bx2, by2 = ep_xy[j]
            ...
            if d2 < 1:
                continue
            fwd_t = (dx2 / d2, dy2 / d2)
            # Curvature continuity: incoming and outgoing directions must agree
            if incoming_t is not None:
                if incoming_t[0] * fwd_t[0] + incoming_t[1] * fwd_t[1] < cos_2thr:
                    continue
            score = q + _lookahead(j, fwd_t, depth - 1, visited | {j})
            if score > best:
                best = score
        return best

    # Score all candidate bridges with forward lookahead
    candidates = []
    for i in range(N):
        for (j, q) in adj[i]:
            if j <= i:
                continue  # avoid duplicate (i,j)/(j,i) pairs
            ax, ay = ep_xy[i]
            bx, by = ep_xy[j]
            dx, dy = bx - ax, by - ay
            d      = math.sqrt(dx * dx + dy * dy)
            fwd_t  = (dx / d, dy / d) if d > 1e-6 else None
            fwd    = _lookahead(j, fwd_t, DEPTH, {i, j})
            total  = q + fwd
            candidates.append((total, i, j))

    candidates.sort(reverse=True)

    # Commit bridges greedily from best score; each endpoint used at most once
    result   = binary.copy()
    consumed = set()
    ...
    for (_, i, j) in candidates:
        if i in consumed or j in consumed:
            continue
        cv2.line(result, ep_xy[i], ep_xy[j], 255, 1)
        consumed.add(i)
        consumed.add(j)
...
def _refine_hed_map_with_gradient_mask(bgr_frame, edge_map,
                                       low_mask_th=0.10, high_mask_th=0.30,
                                       gain_high=1.5, gain_mid=1.0, gain_low=0.2):
    """Apply Han & Zhong gradient mask filtering to the HED probability map."""
...
def interior_edgemap(mask_bool, edge_map, frame_w, frame_h,
                     ...
                     threshold_high=None,
                     bridge_kernel=8, bridge_angle=30.0, bridge_lookback=5,
                     bridge_soft_th=0.15, bridge_co_circ=10.0,
    ...
    preprocess='hed':
        Hysteresis on the soft map followed by morphological thinning.
        HED produces very wide soft ridges with semantically meaningful
        weak edges in dark regions.  NMS does NOT work here -- gradient
        direction at HED ridge centres is unreliable (magnitude near zero)
        and NMS would fragment the ridges into dots.  Instead: hysteresis
        on the original soft map uses 8-connectivity to keep weak chains
        (above `threshold`) that lead back to anchor pixels (above
        `threshold_high`), producing connected thick blobs spanning the
        full edge length.  Zhang-Suen thinning then collapses each blob
        to a 1-pixel skeleton; because Zhang-Suen is topology-preserving,
    ...
    if preprocess == 'hed':
        source_frame = original_bgr if original_bgr is not None else bgr_frame
        if source_frame is None:
            source_frame = np.stack((mask_bool.astype(np.uint8) * 255,) * 3, axis=-1)

        refined = _refine_hed_map_with_gradient_mask(source_frame, edge_float_raw)
        if debug_prefix:
            cv2.imwrite(f'{debug_prefix}_02_gradient_mask.png',
                        (refined * 255).astype(np.uint8))

        thinned_f = _nms_edge_map(refined)
        if debug_prefix:
        ...
        if bridge_kernel > 1:
            binary = _directed_endpoint_bridge(
                binary, refined, bridge_kernel, bridge_angle,
                bridge_lookback, bridge_soft_th, bridge_co_circ)
            if debug_prefix:
                cv2.imwrite(f'{debug_prefix}_03b_bridged.png', binary)
            try:
                binary = cv2.ximgproc.thinning(
                    binary, thinningType=cv2.ximgproc.THINNING_ZHANGSUEN)
            except AttributeError:
                from skimage.morphology import skeletonize as _skeletonize
                binary = (_skeletonize(binary > 0).astype(np.uint8)) * 255
            if debug_prefix:
                cv2.imwrite(f'{debug_prefix}_03c_thinned.png', binary)
        ...
        for m in methods:
            ...
            if m == 'canny':
                ...
                interior = interior_edgemap(
                    ...
                    bridge_angle=args.hed_bridge_angle,
                    bridge_lookback=args.hed_bridge_lookback,
                    bridge_soft_th=args.hed_bridge_soft_th,
                    bridge_co_circ=args.hed_bridge_co_circ,
                    bgr_frame=bgr_frame_arg,
                    original_bgr=bgr,
                    color_step=args.color_step,
                    color_patch=args.color_patch,
